...
from ..utils.database import extract_schema, remove_on_clause


sys.path.append('.')
os.environ["TOKENIZERS_PARALLELISM"] = "true"
LEMMATIZER = WordNetLemmatizer()
os.environ["JIEBA_CACHE"] = "/home/kyxu/.jieba_cache"
jieba.initialize()
...
def span_alignment_fix(question: str, alignment: List[Dict[str, str]], schema: Dict[str, List[str]]) -> List[Dict[str, str]]:
    ...
    def fix_question(question: str, alignment: List[Dict[str, str]]) -> List[Dict[str, str]]:
        ...
        alignment_dict = {
            a['token'].lower(): a["schema"] for i, a in enumerate(alignment) if 'token' in a and a['token']
        }
        alignment_types = {
            a["token"].lower(): a["type"] for i, a in enumerate(alignment) if 'token' in a and a['token']
        }
        question_idx, alignment_idx = longest_common_subsequence(
            question_tokens, alignment_tokens)
        # print(question_idx, alignment_idx)

        result: List[Dict[str, str]] = []
        for i, q in enumerate(question_tokens):
            if i not in question_idx:
                match_token = [key for key in alignment_dict.keys() if q in key]
                if match_token:
                    result.append({
                        "token": q,
                        "schema": str(alignment_dict[match_token[0]]),
                        "type": str(alignment_types[match_token[0]])
                    })
                else:
                    result.append({
                        "token": q,
            ...
            # print(q)
            # print(alignment[alignment_idx[idx]])
    ...
    def get_span_length(span: str, idx: int, alignment: List[Dict[str, str]]) -> int:
        ...
        span_length = len(list(jieba.cut(question)))
...
def generate_prompt(data_demo_pair):
    ...
    while prompt_demos and ((len(tokenizer(prompt)['input_ids']) if tokenizer else len(word_tokenize(prompt))) > (max_position_embeddings - 64)):
...
if __name__ == '__main__':
    ...
    import argparse
    demo_args = argparse.Namespace(selector_type="bm25", demonstration_number=1)
    demonstrations = select_multiple(
        data_dev, data_train, demo_args)
