...
def pack_question(data: Dict[str, Any], use_alignment: str = "", mode: str = 'ch') -> str:
    if use_alignment and data['alignment']:
        alignment: List[Dict[str, str]] = data['alignment'][use_alignment]
        ...
        for i, a in enumerate(alignment):
            ...
            schema = a['schema']
            token_type = a['type']
            if not token_type or token_type.lower() == 'none' or not schema or schema.lower() == 'none':
                result.append(token)
                continue
            
            # 如果当前 token 有 schema，且与下一个 token 的 schema 不同
            if schema and (i == len(alignment) - 1 or alignment[i + 1]['schema'] != schema):
                result.append(f"{token}({schema})")
            else:
                result.append(token)
...
if __name__ == '__main__':
    ...
    print(f"select multiple from {len(data_dev)} data")
    import argparse
    ...
    prompts = [generate_prompt(data_demo_pair) for data_demo_pair in data_demo_pairs]
    print(prompts[0])
    ...
    for d, p, x in zip(data_dev, predictions, prompts):
        ...
        d['prediction'] = {
            "text": prediction,
            "query": extract_sql_from_text(prediction)
        }
