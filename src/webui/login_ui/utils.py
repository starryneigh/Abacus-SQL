...
def check_huozi_pass(username: str, password: str) -> bool:
    """
    ...
    login_url = "https://huozi.8wss.com/api/login"
    login_data = {"name": username, "password_hash": password_hash}

    response = requests.post(login_url, json=login_data, headers=headers)

    if response.status_code == 200:
        print(response.text)  # 输出响应内容为字符串
...
def check_usr_pass(username: str, password: str, type="") -> bool:
    """
    Authenticates the username and password.
    """
    save_path = "_secret_auth_" + type + ".json"
    with open(save_path, "r") as auth_json:
...
def register_new_usr(
    ...
    with open(save_path, "w") as auth_json_write:
